...
-->
<template>
	<div id="details" v-if="productDetails != '' && productPicture != ''">
		<!-- 头部 -->
		...
		<div class="main">
			...
			<div class="block">
				<el-carousel height="560px" v-if="productPicture.length > 1">
					<el-carousel-item v-for="item in productPicture"
						:key="item.id" style="background: none;">
						<img style="height:560px;" :src="item.productPicture"
							:alt="item.intro" />
					</el-carousel-item>
				</el-carousel>
			...
			<div class="content">
				...
				<div class="price">
					...
					<span
						...
						class="del">{{ productDetails.productPrice }}元</span>
				</div>
				<div class="pro-list">
					<span class="pro-name">{{ productDetails.productName
						}}</span>
					<span class="pro-price">
						<span>{{ productDetails.productSellingPrice }}元</span>
						<span
							v-show="productDetails.productPrice != productDetails.productSellingPrice"
							class="pro-del">{{ productDetails.productPrice
							}}元</span>
					</span>
					<p class="price-sum">总计 :
						{{ productDetails.productSellingPrice }}元</p>
				</div>
				<!-- 内容区底部按钮 -->
				<div class="button">
					<el-button class="shop-cart" :disabled="dis"
...
export default {
	...
	watch: {
		...
		productID: async function (val) {
			...
			await this.getDetailsPicture(val);
		},
	},
	methods: {
