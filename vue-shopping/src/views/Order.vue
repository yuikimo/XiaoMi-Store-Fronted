			...
			<div class="order-header-content">
				<p>
					<i class="el-icon-s-order"
						style="font-size: 30px;color: #ff6700;"></i>
					我的订单
				</p>
			</div>
		</div>
		...
		<div class="order-content" v-if="orders.length > 0">
			<div class="content" v-for="(item, index) in orders" :key="index">
				<ul>
					...
					<li class="order-info">
						...
						<div class="order-time">订单时间: {{ item[0].orderTime |
			dateFormat }}</div>
					</li>
					<li class="header">
					...
					<!-- 订单列表 -->
					<li class="product-list" v-for="(product, i) in item"
						:key="i">
						<div class="pro-img">
							<router-link
			...
			product.productName }}</router-link>
						...
						<div class="pro-total pro-total-in">
							{{ product.productPrice * product.productNum }}元
						</div>
					</li>
				</ul>
				<div class="order-bar">
			...
			total[index].totalNum }}</span>
					...
					<div class="order-bar-right">
						<span>
							...
							<span class="total-price">{{ total[index].totalPrice
								}}元</span>
						</span>
					</div>
					<!-- 订单列表END -->
