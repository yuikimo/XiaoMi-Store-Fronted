	...
	<div id="details">
		...
		<div class="main">
			...
			<div class="content">
				...
				<p class="store">开始时间:
					<span>
						{{ productDetails.startTime | dateFormat }}
					</span>
				</p>
				<p class="store">结束时间:
					<span>
						{{ productDetails.endTime | dateFormat }}
					</span>
				</p>
				<div class="price">
				...
				<div class="pro-list">
					...
					<p class="price-sum">秒杀价 :
						{{ productDetails.seckillPrice }}元
					</p>
...
export default {
	...
	watch: {
		...
		seckillID: async function (val) {
			await this.getDetails(val);
		},
		remainSeconds: async function () {
			// if (val == 0) {
			//     return;
			// }
			...
			await this.$axios
				...
				.then(res => {
					...
					if (this.productDetails.startTime > new Date().getTime()) {
						...
						this.remainSeconds = Math.floor(this.productDetails.startTime / 1000 -
